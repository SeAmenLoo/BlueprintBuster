...
void FBlueprintBusterModule::RegisterMenus()
{
    ...
    Section.AddMenuEntry(
        ...
        LOCTEXT("BlueprintBusterDumpSelectedTooltip", "Dump selected Blueprint assets to BlueprintBuster JSON files."),
        FSlateIcon(),
...
{
    ProcessSelectedBlueprints(false);
}

void FBlueprintBusterModule::ConvertSelectedBlueprintsToCpp()
{
    ProcessSelectedBlueprints(true);
}

void FBlueprintBusterModule::ProcessSelectedBlueprints(bool bGenerateCpp)
{
    ...
    FScopedSlowTask SlowTask(Blueprints.Num(), LOCTEXT("BlueprintBusterWorking", "BlueprintBuster is processing Blueprints..."));
    ...
    for (UBlueprint* Blueprint : Blueprints)
    {
        ...
        if (bGenerateCpp)
        {
            ...
            FString StdErr;
            int32 ReturnCode = 0;
            const bool bOk = BlueprintBusterPython::TranslateDumpToCpp(DumpFilePath, CppDir, ModuleApi, PythonExe, &StdOut, &StdErr, &ReturnCode);
            if (!bOk)
            {
    ...
    FMessageDialog::Open(EAppMsgType::Ok, FText::Format(LOCTEXT("BlueprintBusterDoneFmt", "Processed {0} Blueprint(s)."), FText::AsNumber(Successes)));
}
